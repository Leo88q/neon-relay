...
MACRO_CONFIG_INT(ClShowRecord, cl_showrecord, 0, 0, 1, CFGFLAG_CLIENT | CFGFLAG_SAVE, "Show old style race client records")
...
// http map download
MACRO_CONFIG_STR(ClInfoUrl, cl_info_url, 100, "", CFGFLAG_CLIENT | CFGFLAG_SAVE, "URL of the optional info service (news, latest version, stun servers). Empty disables it completely and no data is transmitted")
MACRO_CONFIG_STR(ClMapDownloadUrl, cl_map_download_url, 100, "", CFGFLAG_CLIENT | CFGFLAG_SAVE, "URL used to download maps (must start with https://). Empty disables HTTP map downloads and the map is transferred by the game server instead")
...
#endif
MACRO_CONFIG_STR(ClSkinDownloadUrl, cl_skin_download_url, 100, "", CFGFLAG_CLIENT | CFGFLAG_SAVE, "URL used to download skins (empty disables skin downloads)")
MACRO_CONFIG_STR(ClSkinCommunityDownloadUrl, cl_skin_community_download_url, 100, "", CFGFLAG_CLIENT | CFGFLAG_SAVE, "URL used to download community skins (empty disables community skin downloads)")
MACRO_CONFIG_INT(ClVanillaSkinsOnly, cl_vanilla_skins_only, 0, 0, 1, CFGFLAG_CLIENT | CFGFLAG_SAVE, "Only show skins available in the vanilla 0.6 game")
...
MACRO_CONFIG_STR(ClPlayerSkin, player_skin, 24, "potato_cool_guy_1", CFGFLAG_CLIENT | CFGFLAG_SAVE | CFGFLAG_INSENSITIVE, "Player skin")
...
MACRO_CONFIG_STR(ClDummySkin, dummy_skin, 24, "potato_cool_guy_1", CFGFLAG_CLIENT | CFGFLAG_SAVE | CFGFLAG_INSENSITIVE, "Dummy skin")
...
MACRO_CONFIG_STR(ClAssetExtras, cl_asset_extras, 50, "default", CFGFLAG_SAVE | CFGFLAG_CLIENT, "The asset for the game graphics that do not come from the vanilla 0.6 game")
...
MACRO_CONFIG_INT(BrIndicateFinished, br_indicate_finished, 0, 0, 1, CFGFLAG_SAVE | CFGFLAG_CLIENT, "Show whether you have finished a map (transmits your player name to the info service configured in cl_info_url; disabled by default)")
...
MACRO_CONFIG_STR(BrCachedBestServerinfoUrl, br_cached_best_serverinfo_url, 256, "", CFGFLAG_SAVE | CFGFLAG_CLIENT, "Do not set this variable, instead create a neonrelay-serverlist-urls.cfg next to settings_neonrelay.cfg to specify all possible serverlist URLs")
...
MACRO_CONFIG_STR(SvMap, sv_map, 128, "Neon Relay Basin", CFGFLAG_SERVER, "Map to use on the server")
...
MACRO_CONFIG_STR(SvRegister, sv_register, 16, "0", CFGFLAG_SERVER, "Register server with master server for public listing, can also accept a comma-separated list of protocols to register on, like 'ipv4,ipv6'")
...
MACRO_CONFIG_STR(SvRegisterUrl, sv_register_url, 128, "", CFGFLAG_SERVER, "Masterserver URL to register to (empty means no registration; set sv_register 0 to disable)")
...
MACRO_CONFIG_INT(SvNeonrelaySigning, sv_neonrelay_signing, 0, 0, 1, CFGFLAG_SERVER, "Sign race-finish reward events for the Neon Relay reward backend (0 = off, which is the default; see docs/REWARD_SECURITY.md)")
MACRO_CONFIG_STR(SvNeonrelaySigningKeyFile, sv_neonrelay_signing_key_file, 256, "", CFGFLAG_SERVER, "Path to a 64-hex-char Ed25519 seed file used to sign reward events; keep it out of logs, backups and repositories")
MACRO_CONFIG_STR(SvNeonrelaySigningOutFile, sv_neonrelay_signing_outfile, 256, "", CFGFLAG_SERVER, "JSONL file that signed reward events are appended to, for pickup by the reward backend")
MACRO_CONFIG_INT(SvNeonrelayRewardPerMatchMicro, sv_neonrelay_reward_per_match_micro, 0, 0, 1000000000, CFGFLAG_SERVER, "Reward amount in micro units proposed per signed finish event; the backend enforces caps and may reject")
...
MACRO_CONFIG_INT(SvSkillLevel, sv_skill_level, 1, SERVERINFO_LEVEL_MIN, SERVERINFO_LEVEL_MAX, CFGFLAG_SERVER, "Difficulty level reported to 0.7 clients (0: Casual, 1: Normal, 2: Competitive)")
...
MACRO_CONFIG_INT(ClDDRaceBindsSet, cl_race_binds_set, 0, 0, 3, CFGFLAG_CLIENT | CFGFLAG_SAVE, "What level the race binds are set to (this is automated, you don't need to use this)")
...
MACRO_CONFIG_STR(SvSqliteFile, sv_sqlite_file, 64, "neonrelay-server.sqlite", CFGFLAG_SERVER, "File to store ranks in case sv_use_sql is turned off or used as backup sql server")
...
// menu background map
MACRO_CONFIG_STR(ClMenuMap, cl_menu_map, 100, "", CFGFLAG_CLIENT | CFGFLAG_SAVE, "Background map in the menu (empty = Night Drive night sky with the striped-sun blob, docs/DESIGN_SYNTHWAVE.md; upstream default was auto)")
...
MACRO_CONFIG_STR(SvClientSuggestion, sv_client_suggestion, 128, "Get the Neon Relay client to use all features on this server!", CFGFLAG_SERVER, "Broadcast to display to players without a Neon Relay client")
MACRO_CONFIG_STR(SvClientSuggestionOld, sv_client_suggestion_old, 128, "Your Neon Relay client is old, please update it!", CFGFLAG_SERVER, "Broadcast to display to players with an old version of the Neon Relay client")
MACRO_CONFIG_STR(SvClientSuggestionBot, sv_client_suggestion_bot, 128, "Your client has bots and can be remotely controlled!\nPlease use another client, for example the official Neon Relay client", CFGFLAG_SERVER, "Broadcast to display to players with a known botting client")
