...
void CMenus::RenderServerbrowserServerList(CUIRect View, bool &WasListboxItemActivated)
{
	...
	Headers.Draw(ColorRGBA(0.06f, 0.09f, 0.16f, 0.96f), IGraphics::CORNER_T, 4.0f);
	CUIRect HeadEdge = {Headers.x, Headers.y, Headers.w, 2.0f};
	HeadEdge.Draw(ColorRGBA(0.30f, 0.89f, 0.97f, 0.85f), IGraphics::CORNER_T, 2.0f);
	Headers.VSplitRight(s_ListBox.ScrollbarWidthMax(), &Headers, nullptr);
	View.Draw(ColorRGBA(0.04f, 0.06f, 0.12f, 0.88f), IGraphics::CORNER_NONE, 0.0f);
...
void CMenus::RenderServerbrowserStatusBox(CUIRect StatusBox, bool WasListboxItemActivated)
{
	...
	if(LoadingProgressionTimeDiff > 0.0f)
	{
		...
		RefreshBar.Draw(ColorRGBA(0.30f, 0.89f, 0.97f, RefreshBarAlpha), IGraphics::CORNER_NONE, 0.0f);
	}
	...
	// buttons
	{
		...
		// connect button
		{
			...
			SMenuButtonProperties Props;
			Props.m_UseIconFont = true;
			Props.m_Color = ColorRGBA(0.06f, 0.09f, 0.16f, 0.92f);
...
void CMenus::RenderServerbrowserFilters(CUIRect View)
{
	...
	{
		const ColorRGBA ColorActive = ColorRGBA(0.06f, 0.09f, 0.16f, 0.96f);
		const ColorRGBA ColorInactive = ColorRGBA(0.04f, 0.06f, 0.12f, 0.75f);
	...
	const std::function<void(int ItemIndex, CUIRect Item, const void *pItemId, bool Active)> &RenderItem)
{
	...
	for(int ItemIndex = 0; ItemIndex < MaxItems; ++ItemIndex)
	{
		...
		if(Ui()->HotItem() == pItemId && !ScrollRegion.Animating())
			Item.Draw(ColorRGBA(0.30f, 0.89f, 0.97f, 0.22f), IGraphics::CORNER_ALL, 4.0f);
...
void CMenus::RenderServerbrowserCommunitiesFilter(CUIRect View)
{
	...
	Tab.Draw(ColorRGBA(0.06f, 0.09f, 0.16f, 0.96f), IGraphics::CORNER_T, 4.0f);
	CUIRect TabEdge = {Tab.x, Tab.y, Tab.w, 2.0f};
	TabEdge.Draw(ColorRGBA(0.30f, 0.89f, 0.97f, 0.75f), IGraphics::CORNER_T, 2.0f);
	Ui()->DoLabel(&Tab, Localize("Communities"), 12.0f, TEXTALIGN_MC);
	View.Draw(ColorRGBA(0.04f, 0.06f, 0.12f, 0.88f), IGraphics::CORNER_B, 4.0f);
...
void CMenus::RenderServerbrowserFriends(CUIRect View)
{
	...
	for(size_t FriendType = 0; FriendType < NUM_FRIEND_TYPES; ++FriendType)
	{
		...
		Header.Draw(ColorRGBA(0.06f, 0.09f, 0.16f, Ui()->HotItem() == &s_aListExtended[FriendType] ? 0.92f : 0.85f), IGraphics::CORNER_ALL, 4.0f);
...
{
	ToolBox.Draw(ColorRGBA(0.05f, 0.08f, 0.14f, 0.94f), IGraphics::CORNER_B, 4.0f);
	CUIRect ToolEdge = {ToolBox.x, ToolBox.y, ToolBox.w, 2.5f};
	ToolEdge.Draw(ColorRGBA(0.30f, 0.89f, 0.97f, 0.70f), IGraphics::CORNER_T, 2.0f);
