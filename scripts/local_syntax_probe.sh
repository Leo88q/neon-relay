...
# The cxx-generated Rust bridge header is only produced by cargo. Provide a minimal
# declaration stub so that the C++ call sites can still be syntax-checked; this is
# logged loudly because it is NOT the real generated header.
mkdir -p "${GEN_DIR}/src/engine/shared"
cat > "${GEN_DIR}/src/engine/shared/rust_version.h" <<'STUB'
// Stub generated by scripts/local_syntax_probe.sh — NOT the cxx-generated header.
// It only declares the two symbols the C++ call sites use, so that translation units
// which include <engine/shared/rust_version.h> can be syntax-checked without cargo.
#ifndef ENGINE_SHARED_RUST_VERSION_STUB_H
#define ENGINE_SHARED_RUST_VERSION_STUB_H
class IConsole;
void RustVersionPrint(IConsole &Console);
void RustVersionRegister(IConsole &Console);
#endif
STUB
echo "  note  wrote a stub for src/engine/shared/rust_version.h (real header needs cargo)"
echo
...
# Headers that only exist when the full dependency stack is installed. A translation
# unit that fails on one of these is reported as "skipped", not as a failure.
MISSING_DEP_REGEX='rust_version\.h|sqlite3\.h|curl/|openssl/|cxx\.h|SDL\.h|SDL2/|libnotify/|vulkan/|GL/|freetype|png\.h|ogg/|opus|wavpack|discord_game_sdk|miniupnpc|mysql|websockets|avcodec|generated/wordlist\.h'

# Client-side translation units that do not pull in SDL/Vulkan/FreeType. They are
# probed as well because the rebrand touched several of them.
CLIENT_PROBE_FILES="
...
src/game/client/components/menus_settings_credits.cpp
src/game/client/components/menus_settings_language.cpp
src/game/client/components/menus_settings_appearance.cpp
src/game/client/components/menus_settings_ddnet.cpp
...
for f in $CLIENT_PROBE_FILES; do
	[ -f "$f" ] && PROBE_FILES="$PROBE_FILES $f"
done
...
for f in $PROBE_FILES; do
	if "$CXX" -std="$STD" -fsyntax-only "${INCLUDES[@]}" "$f" 2>"${GEN_DIR}/last-error.txt"; then
		...
		if grep -qE "$MISSING_DEP_REGEX" "${GEN_DIR}/last-error.txt"; then
			skipped=$((skipped + 1))
