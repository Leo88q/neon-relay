...
cmake -S . -B "$BUILD" -GNinja -DCMAKE_BUILD_TYPE=Release \
  ...
  -DNEONRELAY_WARMUP_PROBE=ON -DNEONRELAY_DM_PROBE=ON
cmake --build "$BUILD" --target game-client game-server --parallel 2
xvfb-run -a -s '-screen 0 1400x1200x24' python3 scripts/test_neon_dm_server.py "$BUILD/neonrelay" "$BUILD/neonrelay-server" "${RUNNER_TEMP:-/tmp}/client-preview"
