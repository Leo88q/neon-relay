...
EMOTE_ROW = [  # 16x16 sheet: first 6 rows carry the emote glyphs used in-game
    [("face", "smile", "normal", CYAN), ("face", "grin", "normal", CYAN), ("face", "smile", "wink", CYAN), ("face", "flat", "shut", VIOLET),
     ("face", "sad", "normal", VIOLET), ("face", "open", "angry", MAGENTA), ("face", "open", "normal", MAGENTA), ("face", "smile", "heart", MAGENTA),
     ("face", "grin", "shut", CYAN), ("face", "sad", "wink", VIOLET), ("face", "flat", "normal", DIM), ("face", "open", "shut", MAGENTA),
     ("face", "tongue", "wink", CYAN), ("face", "smile", "shut", VIOLET), ("face", "grin", "angry", MAGENTA), ("face", "flat", "wink", DIM)],
]
EMOTE_SYMBOLS = ["heart", "star", "bolt", "note", "drop", "flame", "anger", "zzz"]
...
def build_emoticons():
    ...
    for col in range(16):
        b = (col * 32 * SS + 3 * SS, 3 * SS, col * 32 * SS + 29 * SS, 29 * SS)
        kind, arg, eyes, color = EMOTE_ROW[0][col]
        face(d, b, arg, eyes, color)
    for row in range(1, 16):
        for col in range(16):
            sym = EMOTE_SYMBOLS[(row * 16 + col) % len(EMOTE_SYMBOLS)]
            color = (CYAN, MAGENTA, VIOLET, WHITE)[(row + col) % 4]
            b = (col * 32 * SS + 4 * SS, row * 32 * SS + 4 * SS, col * 32 * SS + 28 * SS, row * 32 * SS + 28 * SS)
            symbol(d, b, sym, color)
...
def icon_glyph(d, b, name, color):
    ...
    if name == "gear":
        ...
        glow_dot(d, cx, cy, r * 0.28, color, steps=4)
    elif name == "monitor":
        ...
        circle(d, [cx - r * 0.8, cy - r * 0.8, cx + r * 0.8, cy + r * 0.8], col, wd)
        glow_dot(d, cx, cy - r * 0.38, r * 0.14, color, steps=3)
...
GUI_ICON_NAMES = ["gear", "person", "people", "globe", "flag", "speaker", "check", "cross", "arrow", "heart", "star", "shield",
                  "key", "lock", "chat", "eye", "monitor", "coin", "trophy", "info", "grid", "wrench", "palette", "music"]
...
def build_gui_icons():
    ...
    for row in range(2):
        for col in range(12):
            name = GUI_ICON_NAMES[row * 12 + col]
            color = WHITE if row == 0 else CYAN
            b = (col * 32 * SS + 5 * SS, row * 32 * SS + 5 * SS, col * 32 * SS + 27 * SS, row * 32 * SS + 27 * SS)
