#!/usr/bin/env python3
"""Development-only dm7 visual adaptation – v3 substantially bigger with photo background.
...
def build(folder):
    ...
    m.info.version='chrome-v3'
    m.info.credits='Modified dm7; atrium v3 520x180 with freeze, obstacles, all weapons and photo background by Neon Relay'
    m.info.license='CC-BY-SA 3.0'
    m.info.settings=list(source.info.settings)
    for name in ('chrome','pastel','ring','bevels'):
    ...
    bg=m.groups.new(); bg.name='Sky'; bg.parallax_x=0; bg.parallax_y=0
    sky=bg.layers.new_quads(); sky.name='Atrium'; sky.image=1
    q=quad(sky,-100,-70,200,160)
    q.position_env=env('Sky drift','Position',[(0,(0,0,0)),(12000,(.6,-.4,0)),(24000,(0,0,0))])
    ...
    for k in range(18):
        ...
        for i in range(28):
            ...
            x0=cx+7*math.cos(a0); y0=24-12*math.sin(a0)
            x1=cx+7*math.cos(a1); y1=24-12*math.sin(a1)
            q=quad(ribs,x0,y0,.1,.1)
            q.corners=[(x0,y0),(x1,y1),(x0+.13,y0+.12),(x1+.13,y1+.12)]
            q.colors=[(94,118,176,28)]*4
        for x0 in (cx-7,cx+7):
            q=quad(ribs,x0,24,.12,38); q.colors=[(94,118,176,25)]*4

    for lane in range(4):
        g=m.groups.new(); g.name='Rings '+str(lane); g.parallax_x=8+lane*6; g.parallax_y=8+lane*6
        layer=g.layers.new_quads(); layer.name='Film rings'; layer.image=2
        for i in range(20):
            q=quad(layer,i*8-30,((i*7+lane*11)%50)-12,1.5+lane*.35,1.5+lane*.35)
    ...
    actual_front=reopened.front_layer().tiles if reopened.front_layer() else None
    assert np.array_equal(actual[:,:,0],game[:,:,0]), 'Collision/entity IDs changed'
    occupied=game[:,:,0]!=0
    assert np.array_equal(actual[:,:,1][occupied],game[:,:,1][occupied])
    assert actual_front is not None and np.array_equal(actual_front[:,:,0],front[:,:,0]), 'Front freeze changed'
    ...
    report={'status':'expanded atrium v3 520x180 with freeze, obstacles, all 6 weapons and photo background',
            ...
            'old_region_preserved_except_junctions':True,'junctions':JUNCTIONS,
            'added_spawns_v2':NEW_SPAWNS_V2,'added_spawns_v3':NEW_SPAWNS_V3,
            'front_freeze_tiles':int(np.count_nonzero(front[:,:,0]==9)),
            'candidates':[audit(p) for p in sorted((ROOT/'data/maps').glob('dm*.map'))],
            'empty_tile_flags_normalized':empty_flag_changes,'raw_game_bytes_identical':False,
            ...
            'photo_background':True,'photo_source':'.cache/chrome-dm/photo_bg.jpg (AI original)'}
