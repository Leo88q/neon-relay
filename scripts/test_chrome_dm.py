#!/usr/bin/env python3
"""Structural checks for v5 dark neon with space, visible freeze, meteors, many jumps."""
import hashlib
import json
...
from chrome_dm_geometry import extend, front_layer, change_mask, NEW_SPAWNS_V2, NEW_SPAWNS_V3, FREEZE_ZONES, SMALL_JUMPS
...
class ChromeDmTest(unittest.TestCase):
    def test_extension_and_original_region(self):
        ...
        # front freeze visible
        front=front_layer()
        self.assertEqual(front.shape,(180,520,2))
        self.assertGreater(np.count_nonzero(front[:,:,0]==9), 100)
        # small jumps exist
        self.assertGreater(len(SMALL_JUMPS), 30)
        # Air connectivity for spawns
    ...
    def test_build_roundtrip_and_surfaces(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('FreezeWalls', layers)
            self.assertIn('Meteors', layers)
