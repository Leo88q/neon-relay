...
- [ ] No private key, seed phrase, treasury key or production mint address added (`scripts/check_secrets.py --self-test && scripts/check_secrets.py`)
- [ ] Reward logic is server-authoritative — no reward decision derived from client input alone (`docs/REWARD_SECURITY.md`)
- [ ] Reward writes are idempotent (`idempotency_hash` = SHA256 over the canonical signed event bytes — `docs/REWARD_SECURITY.md` §3)
